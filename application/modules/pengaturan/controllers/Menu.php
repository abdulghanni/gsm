...
class Menu extends MX_Controller {
	...
	function get_column(){
		...
		$colModel['id_parents'] = array('Parents',110,TRUE,'left',2);
		$colModel['title'] = array('Title',110,TRUE,'left',2);
		$colModel['filez'] = array('Filez',110,TRUE,'left',2);
		$colModel['icon'] = array('Icon',110,TRUE,'left',2);
		$colModel['sort'] = array('Sort',110,TRUE,'left',2);
		$colModel['is_active'] = array('Active?',110,TRUE,'left',2);
		$colModel['edit'] = array('Action',110,TRUE,'left',2);
	...
	function get_record(){
		...
		{/*
			...
			$row->sort,
			$row->is_active,
			"<a href='".base_url().$this->module.'/'.strtolower($this->file_name)."/input/".$row->id."'>Edit</a> | <a href='#'>Delete</a>",
			);
		}
    ...
    function input($id=0)
    {
        ...
        permissionUser();
		$this->data['modul']=GetAll('menu');
		$this->data['opt_parents']=GetOptAll('menu','-Parents-');
		if($id>0){
			$this->data['nm']=GetAll($this->file_name,array('id'=>'where/'.$id))->row_array();
		}
...
function update(){
		...
		foreach($GetColumns as $r)
		{
			...
			$data['is_active']=$this->input->post('is_active')? 'Active':'inActive';
		echo $id;
		...
		redirect($this->module.'/'.$this->file_name);
}
