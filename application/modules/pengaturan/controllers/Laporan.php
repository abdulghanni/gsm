...
class Laporan extends MX_Controller {
	...
	function get_record(){
		...
		{/*
			...
			$row->id,
			$row->id,
			"<a class='btn btn-sm btn-light-azure' href='".base_url()."pengaturan/laporan/input/".$row->id."' title='detail'>".$row->username."</i></a>",
			$row->full_name,
			$row->name
			);
		}
    ...
    function input($id=NULL)
    {
		...
		$this->data['modul']=GetAll('menu',array('id_parents'=>'where/0'));
                $this->data['laporan']=GetAll('report');
