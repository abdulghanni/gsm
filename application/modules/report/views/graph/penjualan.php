<form method="post" action="<?php echo site_url('graph_penjualan').'/view'?>"><tr id="periodmonth">
	<td>Period</td>
	<td>:</td>
	<td>
...
$(function () {
    $('#laba').highcharts({
        ...
        title: {
            text: 'Penjualan - <?php echo getBulan(substr($fulldate,5,2)).' '.substr($fulldate,0,4) ?>'
        },
        yAxis: { 
            allowDecimals: false,
            title: {
    ...
    <?php
	 for($a=1;$a<=substr($fulldate,8,2);$a++){
		...
		<tr>
			<th><?php echo 'Tgl '.$a;
			?></th>
			<td><?php echo GetPenjualanHarian($f[0].'-'.$f[1].'-'.$s)?></td>
		</tr>
        <?php } ?>
	</tbody>
</table>
