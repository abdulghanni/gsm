...
class Penyesuaian extends MX_Controller {
	...
	function get_column(){
		...
		$colModel['gudang'] = array('Asal',110,TRUE,'left',2);
	...
	function get_flexigrid()
	{
		...
		$this->db->select("a.id as id,a.ref as ref,b.title as gudang,c.title as gudang_to,a.tgl as tgl,a.created_on")->from('stok_Penyesuaian a');
		$this->db->join('gudang b','b.id=a.gudang','left');
	...
	function liststok(){
			$gudang=$_POST['g'];
			$data['list']=GetAll('stok',array('gudang_id'=>'where/'.$gudang,'limit'=>'0/10'));
			$this->load->view('Penyesuaian/input_list',$data);
	}
    function input()
    {
    ...
    function add()
    {
		...
		if($list['jumlah'][$i]>0){
            $data2 = array(
                ...
                'selisih' => str_replace(',', '', $list['selisih'][$i]),
                'satuan_id' => $list['satuan'][$i]
                );
        $this->db->insert($this->module.'_'.$this->file_name.'_list', $data2);
		//keluarstok($data['gudang'],$data2['kode_barang'],$data2['jumlah']);
		//masukstok($data['gudang'],$data2['kode_barang'],$data2['jumlah']);
		$this->db->where(array('gudang_id'=>$data['gudang'],'barang_id'=>$list['kode_barang'][$i]));
		$this->db->update('stok',array('dalam_stok'=>$list['jumlah'][$i]));
		}
        endfor;
		}
