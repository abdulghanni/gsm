...
class Opname extends MX_Controller {
	...
	function get_column(){
		...
		$colModel['gudang_to'] = array('Gudang',110,TRUE,'left',2);
	...
	function get_flexigrid()
	{
		...
		$this->db->select("a.id as id,a.ref as ref,c.title as gudang,a.tgl as tgl,a.created_on")->from('stok_opname a');
		//$this->db->join('gudang b','b.id=a.gudang_from','left');
		$this->db->join('gudang c','c.id=a.gudang','left');
	...
	function loadstok($gdg){
			$data=array();
			$data['stok']=GetAll('stok',array('gudang_id'=>'where/'.$gdg))->result_array();
			//lastq();
			$this->load->view('opname/stokgudang',$data);
	}
}