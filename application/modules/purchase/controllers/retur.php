...
class Retur extends MX_Controller {
    ...
    function input()
    {
        ...
        $this->data['po'] = GetAllSelect('stok_penerimaan', array('id', 'created_on'), array('id'=>'order/desc'))->result();
    ...
    function get_dari_po($id)
    {
        ...
        $this->data['tgl_terima'] = getValue('tgl', 'stok_penerimaan', array('id'=>'where/'.$id));
        $this->data['order'] = $this->main->get_detail_po($order_id);
        $this->data['order_list'] = getAll('stok_penerimaan_list', array('penerimaan_id'=>'where/'.$id));
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $r) {
            ...
            $row[] = date('Ymd', strtotime($r->created_on)).sprintf('%04d',$r->penerimaan_id);
            $row[] = $r->po;
            $row[] = $r->kontak;
            $row[] = $r->tanggal_penerimaan;
