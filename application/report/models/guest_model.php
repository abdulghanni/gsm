...
class Guest_model extends CI_Model {
	...
	var $column = array('ip', 'browser', 'os'); //set column field database for order and search
	var $order = array('tbl_guest.id' => 'desc'); // default order 
		...
		foreach ($this->column as $item) // loop column 
		{
			if($_POST['search']['value'])
			{
				if($item == 'ip'){
					...
					$item = $this->table.'.browser';
				}elseif($item == 'os'){
					$item = $this->table_join1.'.os';
				}
